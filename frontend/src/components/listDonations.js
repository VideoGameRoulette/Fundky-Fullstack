...
const ListDonations = ({ clientid = "0" }) => {
  ...
  const getDonations = async () => {
    try {
      const { donations } = await fetch("/api/donations/list/" + clientid).then(
  ...
  const getStreamName = (donation) => {
    ...
    return (
      <td className="text-center border border-indigo-300 bg-indigo-500 text-white"></td>
    );
  };
  ...
  const renderPage = () => {
    ...
    return (
      <Fragment key="donations">
        <h1 className="text-center border border-indigo-500 bg-indigo-900 text-white">
          Donations List
        </h1>
